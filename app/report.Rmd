...
params:
  ...
  nf_notes: "These are the pipeline notes. <br> - Point one <br> - Point two"
  multiqc_short: FALSE
  multiqc_notes: "These are the MultiQC report notes."
  # the absolute path to output folders of DE analysis
  report_out_dir: "/Volumes/Projects/bulkRNAseq_demo/"
  # the absolute path to the nf-core/rnaseq output folder;
  nextflow_out_dir: "/Volumes/Projects/bulkRNAseq_demo/results"
  # the absolute path to the gtf file used in the nf-core/rnaseq run
  gtf_file: "/Volumes/Projects/bulkRNAseq_demo/gencode.v38.primary_assembly.annotation.gtf"
  ...
  metadata_file: "/Volumes/Projects/bulkRNAseq_demo/metadata.txt"
  ...
  ora_msigdbr_category: "C2"
  ora_msigdbr_subcategory: "CP:KEGG"
...
if(params$multiqc_short) {
  system(paste("find", params$nextflow_out_dir, "-name \"._*\" -type f -delete"))
  system(paste("multiqc", params$nextflow_out_dir, "-c /app/multiqc_config.yml -o", file.path(params$report_out_dir, "2.Pre-processing_of_raw_reads"), "-f"))
}else{
  suppressMessages(file.copy(file.path(params$nextflow_out_dir, "multiqc", "star_salmon", "multiqc_report.html"), file.path(params$report_out_dir, "2.Pre-processing_of_raw_reads/multiqc_report.html"), overwrite = T))
}
```
...
if(params$gsea_msigdb){
  ...
  for (comp in my_contrasts) {
    ...
    cat(paste0('![](', params$report_out_dir, '/4.Functional_analysis/', comp, '_GSEA_plot_', paste(c(params$gsea_msigdbr_category, params$gsea_msigdbr_subcategory), collapse = '_'), '.png)'))
    cat(paste0('![](', params$report_out_dir, '/4.Functional_analysis/', comp, '_GSEA_bubble_plot_', paste(c(params$gsea_msigdbr_category, params$gsea_msigdbr_subcategory), collapse = '_'), '.png)'))
    cat('</div>')
  }
}
