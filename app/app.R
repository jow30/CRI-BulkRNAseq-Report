  ...
  fluidRow(
    column(12,
           ...
           selectInput("report_out_dir", "Output Directory for DE Analysis:",
                       choices = c(Sys.getenv("PWD"), grep("\\/\\.", list.dirs(path = Sys.getenv("PWD"), full.names = TRUE, recursive = TRUE), invert = TRUE, value = TRUE)), width = "100%"),
           selectInput("nextflow_out_dir", "Nextflow Output Directory:",
                       choices = c(Sys.getenv("PWD"), grep("\\/\\.", list.dirs(path = Sys.getenv("PWD"), full.names = TRUE, recursive = TRUE), invert = TRUE, value = TRUE)), 
                       selected = grep("\\/\\.", list.dirs(path = Sys.getenv("PWD"), full.names = TRUE, recursive = FALSE), invert = TRUE, value = TRUE)[1], width = "100%"),
           selectInput("gtf_file", "GTF File Used in the nf-core/rnaseq Run:",
                       choices = list.files(path = Sys.getenv("PWD"), pattern = ".gtf", full.names = TRUE, recursive = TRUE), width = "100%"),
           helpText("The protein-coding gene filtering is based on the gene_type attribute at the 9th column of the GTF file. So, if you didn't use Gencode references, the GTF format could be incompatible with this program and the protein-coding gene filtering won't work properly."),
           ...
           selectizeInput("sample_remove", "Samples to Remove:", choices = NULL, multiple = TRUE, options = list(create = TRUE), width = "100%"),
