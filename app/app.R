  ...
  fluidRow(
    column(12,
           ...
           textAreaInput("nf_notes", "A Breif Description of the Executed Pipeline:", " - We used the nf-core/rnaseq v3.16.0 pipeline for pre-processing of raw reads. \n - We used the STAR->Salmon route for read alignment and quantification. \n - We used the GRCm38 reference genome for read mapping and Gencode vM25 for gene annotation.",
                         width = "100%", height = "200px"),
           checkboxInput("multiqc_short", "Slim Down the MultiQC Report", TRUE),
           helpText("The default multiQC report generated by the nf-core/rnaseq workflow has some confusing and unnecessary information. So we prefer to trim it down to make it easier to understand for clients."),
           helpText("We can customize the multiQC report using the multiqc_config.yml file. It was derived from the default one at https://github.com/nf-core/rnaseq/blob/3.16.0/workflows/rnaseq/assets/multiqc/multiqc_config.yml."),
           helpText("Basically, we made the following modifications: 1) Change column visibility in the General Statistics table; 2) Remove the following sections: samtools-stats, samtools-flagstat, rseqc-bam_stat, Sample relationships. 3) Re-order table columns and sections."),
           ...
           helpText("The metadata file should be a table containing sample information in TXT format."),
           helpText("The first column should match sample names used in nf-core/rnaseq."),
           helpText("A column named \"group\" must be included to define the sample groups for comparison."),
           helpText("A column named \"batch\" must be included to define the batch information if batch correction is needed."),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$submit_btn, {
    ...
    params <- list(
      ...
      multiqc_short = input$multiqc_short,
