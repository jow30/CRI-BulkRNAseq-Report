...
run_GSEA <- function(comp, res, msigdbr_species, msigdbr_category, msigdbr_subcategory, fdr_thres, outDir) {
  ...
  if (nrow(myGSEA.res)>0) {
    ...
    write_csv(myGSEA.df, paste0(outDir, '/GSEA_table_', paste(c(msigdbr_category, gsub(":", "-", msigdbr_subcategory)), collapse = '_'), '_', comp, '.csv'))
    ...
    ggsave(paste0(outDir, '/GSEA_plot_', paste(c(msigdbr_category, gsub(":", "-", msigdbr_subcategory)), collapse = '_'), '_', comp, '.png'), gsea_plot, width = 7, height = 5)
    ...
    ggsave(paste0(outDir, '/GSEA_bubble_plot_', paste(c(msigdbr_category, gsub(":", "-", msigdbr_subcategory)), collapse = '_'), '_', comp, '.png'), bubble_plot, width = 7, height = 5)
